...
export default function JobsTable({ onlyActive = false }: JobsTableProps) {
  ...
  const jobsDict = useMemo(() => {
    ...
    jobs.forEach(job => {
      const gpuIds = job.gpu_ids ?? '';
      // Modal jobs — group by their gpu_ids key (e.g. "modal_A10G")
      if (gpuIds.startsWith('modal_')) {
        if (!(gpuIds in jd)) {
          const gpuLabel = gpuIds.replace('modal_', '').replace('-', ' ');
          jd[gpuIds] = { name: `⚡ Modal · ${gpuLabel}`, jobs: [] };
        }
        if (['queued', 'running', 'stopping'].includes(job.status)) {
          jd[gpuIds].jobs.push(job);
        } else {
      ...
      // Local GPU jobs
      const gpu = gpuList.find(gpu => gpuIds.split(',').includes(gpu.index.toString())) as GpuInfo;
