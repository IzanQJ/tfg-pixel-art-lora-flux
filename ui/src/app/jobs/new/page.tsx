...
export default function TrainingForm() {
  ...
  const [loraOptions, setLoraOptions] = useState<{ value: string; label: string }[]>([]);

  useEffect(() => {
    if (!isSettingsLoaded) return;
    if (datasetFetchStatus !== 'success') return;

    // Todos los datasets del proyecto usan la subcarpeta /images
    const datasetOptions = datasets.map(name => ({
      value: path.join(settings.DATASETS_FOLDER, name, 'images'),
  ...
  // Cargar LoRAs disponibles en outputs/ para el selector de Pretrained LoRA
  useEffect(() => {
    apiClient
      .get('/api/loras')
      .then(res => res.data)
      .then((data: { loras: string[] }) => {
        const opts = [{ value: '', label: '— ninguno —' }].concat(
          data.loras.map(p => ({ value: p, label: p }))
        );
        setLoraOptions(opts);
      })
      .catch(() => setLoraOptions([{ value: '', label: '— ninguno —' }]));
  }, []);
  ...
  // No sobreescribimos training_folder con TRAINING_FOLDER global:
  // cada job tiene su propia carpeta (outputs/iter_XX)
