...
def modal_debug(message: str):
    print_acc(f"[Modal debug] {message}", flush=True)


def modal_cuda_debug(label: str):
    if not torch.cuda.is_available():
        modal_debug(f"{label} | CUDA not available")
        return
    try:
        ...
        modal_debug(
            f"{label} | device={torch.cuda.get_device_name(device_index)} "
            f"allocated={allocated:.2f}GB reserved={reserved:.2f}GB total={total:.1f}GB"
        )
    except Exception as exc:
        modal_debug(f"{label} | CUDA debug failed: {exc}")
...
class StableDiffusion:
    ...
    def load_model(self):
        ...
        if self.model_config.is_xl or self.model_config.is_ssd or self.model_config.is_vega:
            ...
            flux_stage_started_at = time.perf_counter()
            modal_debug(
                "Flux transformer load start: "
                f"transformer_path={transformer_path} subfolder={subfolder} "
                f"base_model_path={base_model_path} dtype={dtype} "
                f"quantize={self.model_config.quantize} qtype={self.model_config.qtype} "
                f"low_vram={self.low_vram} quantize_device={self.quantize_device} "
                f"target_device={self.device_torch}"
            )
            modal_cuda_debug("Before FluxTransformer2DModel.from_pretrained")
            ...
            modal_debug(f"Flux transformer from_pretrained finished in {time.perf_counter() - flux_stage_started_at:.1f}s")
            modal_cuda_debug("After FluxTransformer2DModel.from_pretrained")
            # hack in model gpu splitter
            if self.model_config.split_model_over_gpus:
                modal_debug("Adding Flux model GPU splitter")
            ...
            if not self.low_vram:
                ...
                modal_debug(f"Moving transformer to quantize_device={self.quantize_device}")
                transformer.to(self.quantize_device, dtype=dtype)
                modal_debug(f"Transformer move to quantize_device finished in {time.perf_counter() - move_started_at:.1f}s")
                modal_cuda_debug("After transformer.to(quantize_device)")
            flush()
            modal_cuda_debug("After initial Flux flush")
            ...
            if self.model_config.lora_path is not None:
                ...
                lora_fuse_started_at = time.perf_counter()
                modal_debug(f"Fusing LoRA path={self.model_config.lora_path} low_vram={self.low_vram}")
                modal_cuda_debug("Before LoRA fuse")
                ...
                if self.low_vram:
                    ...
                    modal_debug("Loading LoRA state dict for low_vram split fuse")
                    ...
                    # double blocks
                    modal_debug(f"Fusing LoRA double blocks: {len(double_transformer_lora)} tensors")
                    ...
                    # single blocks
                    modal_debug(f"Fusing LoRA single blocks: {len(single_transformer_lora)} tensors")
                    ...
                    modal_debug("Loading LoRA weights into FluxPipeline")
                    pipe.load_lora_weights(self.model_config.lora_path, adapter_name="lora1")
                    modal_debug("Fusing LoRA weights")
                    pipe.fuse_lora()
                    # unfortunately, not an easier way with peft
                    pipe.unload_lora_weights()
                modal_debug(f"LoRA fuse finished in {time.perf_counter() - lora_fuse_started_at:.1f}s")
                modal_cuda_debug("After LoRA fuse")
            flush()
            modal_cuda_debug("After LoRA/assistant flush")
            ...
            if self.model_config.quantize:
                ...
                modal_debug(f"Quantizing transformer with qtype={self.model_config.qtype}")
                modal_cuda_debug("Before transformer quantize")
                quantize(transformer, weights=quantization_type, **self.model_config.quantize_kwargs)
                modal_debug(f"quantize(transformer) finished in {time.perf_counter() - quantize_started_at:.1f}s")
                freeze(transformer)
                modal_debug("Transformer frozen after quantize")
                transformer.to(self.device_torch)
                modal_cuda_debug("After quantized transformer.to(device)")
            else:
                modal_debug(f"Moving non-quantized transformer to device={self.device_torch}")
                transformer.to(self.device_torch, dtype=dtype)
                modal_cuda_debug("After non-quantized transformer.to(device)")

            flush()
            modal_cuda_debug("After transformer quantize/final move flush")

            scheduler_started_at = time.perf_counter()
            modal_debug(f"Loading scheduler from base_model_path={base_model_path}")
            scheduler = FlowMatchEulerDiscreteScheduler.from_pretrained(base_model_path, subfolder="scheduler")
            modal_debug(f"Scheduler loaded in {time.perf_counter() - scheduler_started_at:.1f}s")
            self.print_and_status_update("Loading VAE")
            vae_started_at = time.perf_counter()
            if self.model_config.vae_path is not None:
                modal_debug(f"Loading custom VAE from {self.model_config.vae_path}")
                vae = load_vae(self.model_config.vae_path, dtype)
            else:
                modal_debug(f"Loading base VAE from {base_model_path}")
                vae = AutoencoderKL.from_pretrained(base_model_path, subfolder="vae", torch_dtype=dtype)
            modal_debug(f"VAE loaded in {time.perf_counter() - vae_started_at:.1f}s")
            flush()
            modal_cuda_debug("After VAE load flush")
            
            self.print_and_status_update("Loading T5")
            t5_started_at = time.perf_counter()
            modal_debug(f"Loading T5 tokenizer/text encoder from {base_model_path}")
            ...
            modal_debug("Moving T5 text encoder to target device")
            text_encoder_2.to(self.device_torch, dtype=dtype)
            modal_debug(f"T5 loaded and moved in {time.perf_counter() - t5_started_at:.1f}s")
            flush()
            modal_cuda_debug("After T5 load flush")

            if self.model_config.quantize_te:
                self.print_and_status_update("Quantizing T5")
                t5_quantize_started_at = time.perf_counter()
                modal_debug(f"Quantizing T5 with qtype={self.model_config.qtype}")
                quantize(text_encoder_2, weights=get_qtype(self.model_config.qtype))
                freeze(text_encoder_2)
                modal_debug(f"T5 quantized in {time.perf_counter() - t5_quantize_started_at:.1f}s")
                flush()
                modal_cuda_debug("After T5 quantize flush")
                
            self.print_and_status_update("Loading CLIP")
            clip_started_at = time.perf_counter()
            modal_debug(f"Loading CLIP text encoder/tokenizer from {base_model_path}")
            text_encoder = CLIPTextModel.from_pretrained(base_model_path, subfolder="text_encoder", torch_dtype=dtype)
            tokenizer = CLIPTokenizer.from_pretrained(base_model_path, subfolder="tokenizer", torch_dtype=dtype)
            text_encoder.to(self.device_torch, dtype=dtype)
            modal_debug(f"CLIP loaded and moved in {time.perf_counter() - clip_started_at:.1f}s")
            modal_cuda_debug("After CLIP load")

            self.print_and_status_update("Making pipe")
            pipe_started_at = time.perf_counter()
            modal_debug("Creating FluxPipeline object")
            Pipe = FluxPipeline
            ...
            modal_debug(f"FluxPipeline object ready in {time.perf_counter() - pipe_started_at:.1f}s")
            modal_cuda_debug("After FluxPipeline creation")
